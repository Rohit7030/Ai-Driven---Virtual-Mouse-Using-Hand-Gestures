...
class Controller:
    ...
    # Add at the top of Controller class
    smooth_x, smooth_y = 0, 0
    alpha = 0.3  # Smoothing factor (0.1-0.5)

    def get_position(hand_result):
        global smooth_x, smooth_y
        point = 9  # Middle finger MCP (stable point)
        sx, sy = pyautogui.size()
        x = int(hand_result.landmark[point].x * sx)
        y = int(hand_result.landmark[point].y * sy)
    ...
    framecount = 0
    prev_hand = None
    pinch_threshold = 0.5  # Increased from 0.3 (less sensitive)

    # In Controller class
    click_pause_frames = 0
    CLICK_COOLDOWN = 10
    ...
    def changesystembrightness():
        ...
        try:
            ...
            if not brightness:  # Handle empty list
                return
            currentBrightnessLv = brightness[0] / 100.0  # Take first display's brightness
            currentBrightnessLv += Controller.pinchlv / 50.0
            currentBrightnessLv = max(0.0, min(1.0, currentBrightnessLv))  # Clamp between 0-100%
    ...
    def handle_controls(gesture, hand_result):  
        """Impliments all gesture functionality with click cooldown."""      
        # Handle click cooldown
        if Controller.click_pause_frames > 0:
            Controller.click_pause_frames -= 1
            return
        ...
        if gesture == Gest.V_GEST:
            ...
            pyautogui.click()
            Controller.flag = False
            Controller.click_pause_frames = Controller.CLICK_COOLDOWN  # Activate cooldown
            ...
            pyautogui.doubleClick()
            Controller.flag = False
            Controller.click_pause_frames = Controller.CLICK_COOLDOWN * 2  # Longer cooldown for double click
...
class GestureController:
    ...
    def start(self):
        ...
        with mp_hands.Hands(max_num_hands = 2,min_detection_confidence=0.6, min_tracking_confidence=0.5, static_image_mode = False ) as hands:
